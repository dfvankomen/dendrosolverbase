...
namespace seq
{
    namespace test
    {
        ...
        bool isSorted_all_pairs(const std::vector<T> &nodes)
        {
            for (unsigned int i = 0; i < nodes.size(); i++)
            {
                for (unsigned int j = i + 1; j < nodes.size(); j++)
                {
                    if (nodes[i] > nodes[j])
                    {
                        std::cout << "\n Local Sort_pair Check failed for: " << nodes[i] << " and " << nodes[j] << std::endl
                                  << std::endl;
                        return false;
                    }
                }
        ...
        bool isComplete(const std::vector<T> &nodes)
        {
            ...
            if (nodes.size() != 0)
            {
                ...
                if (!state)
                {
                    ...
                    std::cout << "vol diff: " << abs(int(volMaxDepth - vol)) << std::endl;
                }
                ...
                for (unsigned int k = 1; k <= k_s; k++)
                {
                    ...
                    if (myY > 0)
                    {
                        ...
                        if (lookUPIndex != UINT_MAX)
                        {
                            ...
                            {
                                if (!rank)
                                    std::cout << "rank: " << rank << "element ID: " << (i - localBegin) << " element : " << inPtr[i] << " key down: " << tmp << " lookup: " << lookUp << " lookupIndex: " << lookUPIndex << std::endl;
                                return false;
                            }
                        }
                    }
        ...
        bool checkE2NMapping(const std::vector<unsigned int> &E2EMap, const std::vector<unsigned int> &E2NMap, const std::vector<T> &allNodes, unsigned int numDirections, unsigned int elementOrder)
        {
            ...
            {
                if (E2NMap[e * numNpE] != UINT_MAX)
                { // This is because we invalidate some ghost element nodes who doesn't have any neighbours.
