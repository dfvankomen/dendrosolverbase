...
inline void fem::seq::matvec(const T *pt_coords, const da *in, unsigned int pBegin, unsigned int pEnd, const unsigned int lev, const unsigned int maxDepth, const unsigned int order, const ot::TreeNode &pOctant, const RefElement *refEl, da *out, unsigned int mVecType)
{
    ...
    unsigned int ijk[3]; // originallly given "register" in front
    unsigned int cnum;
    ...
    unsigned int pt_status; // originallly given "register" in front
    ...
    for (unsigned int p = pBegin; p < pEnd; p++)
    {
        ...
        if (pt_coords[p].zint() > pMid[2])
            ijk[2] = 2;
        if (pt_coords[p].yint() > pMid[1])
            ijk[1] = 2;
        if (pt_coords[p].xint() > pMid[0])
            ijk[0] = 2;
    ...
    for (unsigned int index = 0; index < duplicateCoordIndex.size(); index++)
    {
        ...
        if (pt_coords[duplicateCoordIndex[index]].zint() > pMid[2])
            ijk[2] = 2;
        if (pt_coords[duplicateCoordIndex[index]].yint() > pMid[1])
            ijk[1] = 2;
        if (pt_coords[duplicateCoordIndex[index]].xint() > pMid[0])
            ijk[0] = 2;
    ...
    for (unsigned int child = 0; child < NUM_CHILDREN; child++)
    {
        ...
        else
        {
            ...
            if (bCounts[child] != nPe)
            {
                ...
                if (pNodeCount != nPe)
                    std::cout << "[Error]" << __func__ << " pNodeCount: " << pNodeCount << std::endl;
            ...
            if (bCounts[child] != nPe)
            {
                ...
                for (unsigned int p = pBegin; p < pEnd; p++)
                {
                    if (fem::seq::computeIJK(pt_coords[p], pOctant, dx, logDx, ijk) == PT_INTERNAL_DG_NODE)
                    {
                        ...
                        if (c2pInterp[IDX(ijk[0], ijk[1], ijk[2])])
                            accumStatus[IDX(ijk[0], ijk[1], ijk[2])] = true;
                    }
                }
