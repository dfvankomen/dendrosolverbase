...
namespace dollar
{
    ...
    public:
        struct info
        {
            ...
            inline friend std::ostream &operator<<(std::ostream &os, const info &k)
            {
        ...
        void print(std::ostream &out, const char *tab = ",", const char *feed = "\r\n") const
        {
            ...
            {
                /**/ if (maxlen > it.size())
                    it += std::string(maxlen - it.size(), ' ');
                else if (maxlen < it.size())
                    it.resize(maxlen);
            }
            // }
                ...
                root.tree_walker(
                    [&](const Node<info> &node)
                    {
                        ...
                        out << "{\"name\": \"" << info.title << "\","
                            ...
                            << (unsigned int)(timestamp * 1000 * 1000) << ","
                                                                          "\"dur\": "
                            << (unsigned int)(info.total * 1000 * 1000) << ","
                                                                           "\"cname\": \""
                            << get_color(cpu) << "\""
                                                 ","
                            << "\"args\": {}},\n";
                    ...
                    [&](const Node<info> &node)
                    {
                        ...
                        out << "{\"name\": \"" << info.title << "\","
                            ...
                            << info.tid << ","
                                           "\"ts\": "
                            << (unsigned int)((timestamp + info.total) * 1000 * 1000) << ","
                                                                                         "\"cname\": \""
                            << get_color(cpu) << "\""
                                                 ","
                            << "\"args\": {}},\n";
    ...
    private:
        std::map<std::string, info> counters;
    };
