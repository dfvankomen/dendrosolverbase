            ...
            if (pMaxDepthBit && ((nNodeEnd - nNodeBegin) > 1) && (pNodes[nNodeBegin].getLevel() > lev))
            {
                ...
                for (int i = 0; i < NUM_CHILDREN; i++)
                {
                    ...
                    if (splitterNodes[hindex] != splitterNodes[hindexN])
                        SFC_treeSearch(pKeys, pNodes, splitterKeys[hindex], splitterKeys[hindexN], splitterNodes[hindex], splitterNodes[hindexN], (pMaxDepthBit - 1), pMaxDepth, index);
                }
            }
            else
            {
