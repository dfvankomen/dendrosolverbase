...
namespace ode
{
    namespace solver
    {
            ...
            if (m_uiMesh->isActive())
            {
                ...
                for (unsigned int index = 0; index < nlsm::NLSM_ASYNC_COMM_K; index++)
                {
                    if (m_uiMesh->getGhostExcgTotalSendNodeCount() != 0)
                        m_uiSendNodeBuf[index] = new double[m_uiMesh->getGhostExcgTotalSendNodeCount()];
                    if (m_uiMesh->getGhostExcgTotalRecvNodeCount() != 0)
                        m_uiRecvNodeBuf[index] = new double[m_uiMesh->getGhostExcgTotalRecvNodeCount()];
            ...
            do
            {
                ...
                if (isRefine)
                {
                    ...
                    if (!(m_uiMesh->getMPIRankGlobal()))
                        std::cout << "initial grid iteration : " << iterCount << " old mesh: " << oldElements_g << " new mesh: " << newElements_g << std::endl;
        ...
        void RK4_NLSM::writeToVTU(double **evolZipVarIn, double **constrZipVarIn, unsigned int numEvolVars, unsigned int numConstVars, const unsigned int *evolVarIndices, const unsigned int *constVarIndices)
        {
            ...
            std::function<void(double, double, double, double, double *)> u_x_t = [](double x, double y, double z, double t, double *var)
...
#ifdef DEBUG_RK_SOLVER
                    if (!rank)
                        std::cout << " stage: " << (nlsm::NLSM_RK4_STAGES - 1) << " begin: " << std::endl;
                    for (unsigned int index = 0; index < nlsm::NLSM_NUM_VARS; index++)
                        ot::test::isUnzipNaN(m_uiMesh, m_uiUnzipVar[index]);
#endif
...
#ifdef DEBUG_RK_SOLVER
                    if (!rank)
                        std::cout << " stage: " << (nlsm::NLSM_RK4_STAGES - 1) << " af rhs UNZIP RHS TEST:" << std::endl;
                    for (unsigned int index = 0; index < nlsm::NLSM_NUM_VARS; index++)
                        ot::test::isUnzipInternalNaN(m_uiMesh, m_uiUnzipVarRHS[index]);
#endif
