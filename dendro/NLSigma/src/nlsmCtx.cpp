...
namespace nlsm
{
    NLSMCtx::NLSMCtx(ot::Mesh *pMesh) : Ctx()
    {
        ...
        m_uiEVar = this->create_vec(ts::CTXVType::EVOLUTION, true, false, false, NLSM_NUM_VARS);

        m_uiEUnzip[0] = this->create_vec(ts::CTXVType::EVOLUTION, false, true, false, NLSM_NUM_VARS);
        m_uiEUnzip[1] = this->create_vec(ts::CTXVType::EVOLUTION, false, true, false, NLSM_NUM_VARS);
    ...
    int NLSMCtx::initialize()
    {
        ...
        DendroScalar *var = new double[NLSM_NUM_VARS];
        DendroScalar **zipIn = new DendroScalar *[NLSM_NUM_VARS];
    ...
    int NLSMCtx::rhs(DVec *in, DVec *out, unsigned int sz, DendroScalar time)
    {
        ...
        this->unzip(in[0], m_uiEUnzip[0], NLSM_ASYNC_COMM_K);
        ...
        this->zip(m_uiEUnzip[1], out[0], NLSM_ASYNC_COMM_K);
    ...
    bool NLSMCtx::is_remesh()
    {
        ...
        MPI_Comm comm = m_uiMesh->getMPIGlobalCommunicator();
        this->unzip(m_uiEVar, m_uiEUnzip[0], NLSM_ASYNC_COMM_K);
