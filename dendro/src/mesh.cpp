...
namespace ot
{
    ...
    void Mesh::buildE2EMap(std::vector<ot::TreeNode> &in, MPI_Comm comm)
    {
        ...
        *  third z axis. (back to front)**/
        ...
        for (unsigned int k = 0; k < m_uiKeys.size(); k++)
        {
            ...
            if (!(OCT_FOUND & m_uiKeysPtr[k].getFlag()))
                continue; // Note that some keys might not be found due to absence of diagonal keys at this stage. (but all the keys should be found after diagonal neighbour exchange. )
        ...
        for (unsigned int k = 0; k < m_uiGhostKeys.size(); k++)
        {
            ...
            if ((OCT_FOUND & m_uiKeysPtr[k].getFlag()))
            {
                ...
                result = m_uiKeysPtr[k].getSearchResult();
                if (ownerList->size())
                    assert(m_uiAllElements[result].isAncestor(m_uiGhostKeys[k]) || m_uiAllElements[result] == m_uiGhostKeys[k]); // To check the result found in the treeSearch is correct or not.
        ...
        *  third z axis. (back to front)**/
        ...
        if (!rank)
            std::cout << "E2E mapping Ended" << std::endl;
    }
    ...
    void Mesh::buildE2EMap(std::vector<ot::TreeNode> &in)
    {
        ...
        *  third z axis. (back to front)**/
        ...
        {
            if (!(OCT_FOUND & m_uiKeysPtr[k].getFlag()))
                std::cout << "key: " << m_uiKeysPtr[k] << " not found: " << std::endl;
            assert((OCT_FOUND & m_uiKeysPtr[k].getFlag())); // Note that all the keys should be found locally due to the fact that we have exchanged ghost elements.
            ownerList = m_uiKeysPtr[k].getOwnerList();
            stencilIndexDirection = m_uiKeysPtr[k].getStencilIndexDirectionList();
            result = m_uiKeysPtr[k].getSearchResult();
...
#define IDXp(i, j, k) k *(eleOrder + 1) * (eleOrder + 1) + j *(eleOrder + 1) + i
        ...
        if (!m_uiActiveRank)
            std::cout << "E2N Mapping ended" << std::endl;
    }
    ...
    void Mesh::buildE2NMap()
    {
        ...
        for (unsigned int e = 0; e < m_uiAllElements.size(); e++)
        {
            ...
            for (unsigned int k = 0; k < m_uiNumDirections; k++)
            {
                ...
                if (m_uiE2EMapping[e * m_uiNumDirections + k] != LOOK_UP_TABLE_DEFAULT)
                    assert(m_uiE2EMapping[e * m_uiNumDirections + k] < m_uiAllElements.size());
            }
        ...
        if (!m_uiActiveRank)
            std::cout << "Invalid nodes removed " << std::endl;
        ...
        //MPI_Barrier(MPI_COMM_WORLD);
        if (!m_uiActiveRank)
            std::cout << "m_uiActiveRank: " << m_uiActiveRank << "Number of actual nodes: " << (E2N_DG_Sorted.size()) << std::endl;
#endif
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        if (m_uiActiveRank)
            std::cout << " DG to CG index updated " << std::endl;
#endif
        ...
        //MPI_Barrier(MPI_COMM_WORLD);
        if (!m_uiActiveRank)
            std::cout << "[NODE] rank:  " << m_uiActiveRank << " pre ( " << m_uiNodePreGhostBegin << ", " << m_uiNodePreGhostEnd << ") local ( " << m_uiNodeLocalBegin << ", " << m_uiNodeLocalEnd << ")"
                      << " post (" << m_uiNodePostGhostBegin << " , " << m_uiNodePostGhostEnd << ")" << std::endl;
        if (!m_uiActiveRank)
            std::cout << "[ELEMENT] rank:  " << m_uiActiveRank << " pre ( " << m_uiElementPreGhostBegin << ", " << m_uiElementPreGhostEnd << ") local ( " << m_uiElementLocalBegin << ", " << m_uiElementLocalEnd << ")"
                      << " post (" << m_uiElementPostGhostBegin << " , " << m_uiElementPostGhostEnd << ")" << std::endl;
    ...
    void Mesh::computeNodalScatterMapDG(MPI_Comm comm)
    {
        ...
        if (npes <= 1)
            return; // nothing to do in the sequential case. (No scatter map required.)
    ...
    void Mesh::computeNodeScatterMaps(MPI_Comm comm)
    {
        ...
        for (unsigned int ele = m_uiElementLocalBegin; ele < m_uiElementLocalEnd; ele++)
        {
            ...
            for (unsigned int i = 0; i < m_uiElementOrder + 1; i++)
                for (unsigned int j = 0; j < m_uiElementOrder + 1; j++)
                    for (unsigned int k = 0; k < m_uiElementOrder + 1; k++)
                    {
                        if (((i > 1) && (i < (m_uiElementOrder - 1))) || ((j > 1) && (j < (m_uiElementOrder - 1))) || ((k > 1) && (k < (m_uiElementOrder - 1))))
                            continue;
        ...
        if (!rank)
            treeNodesTovtk(splitterNodes, rank, "splitterNodes");
        if (!rank)
            treeNodesTovtk(splitterElements, rank, "splitterElements");
        ...
        {
            std::cout << RED << "[Error]"
                      << " m_uiActiveRank: " << m_uiActiveRank << " Total Ghost Elements allocated: " << allocatedGhostNodes.size() << " Number of elements will get recieved: " << (m_uiRecvNodeOffset[npes - 1] + m_uiRecvNodeCount[npes - 1]) << NRM << std::endl;
            assert(false);
        }
        ...
        for (unsigned int k = 0; k < allocatedGhostNodes.size(); k++)
        {
            ...
            {
                std::cout << RED << "[ERROR]: "
                          << "m_uiActiveRank : " << m_uiActiveRank << " allocated [" << k << "]: " << allocatedGhostNodes[k] << " recieved[" << k << "]: " << recvNodeKeys[k] << std::endl;
                assert(false);
            }
    ...
    void Mesh::computeNodalScatterMap1(MPI_Comm comm)
    {
        ...
        if (!rank)
            std::cout << "a2a_ 1 time max: " << t_stat_g << std::endl;
        if (!rank)
            std::cout << "a2a_ 1 sz  min mean max : " << stat_sz[0] << ", " << stat_sz[1] << ", " << stat_sz[2] << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " unzip time: " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " search time: " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " send sm time: " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << "a2a_ 2 time max: " << t_stat_g << std::endl;
        if (!rank)
            std::cout << "a2a_ 2 sz  min mean max : " << stat_sz[0] << ", " << stat_sz[1] << ", " << stat_sz[2] << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " sort time: " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " sm recv update time: " << t_stat_g << std::endl;
    ...
    void Mesh::computeNodalScatterMap3(MPI_Comm comm)
    {
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " local node generation + sort time (max) (s): " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " ghost element ownership build time (max) (s): " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " allocated node generation + sort time (max) (s): " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " allocated1 ownership computation (max) (s): " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " allocated node zip time (max) (s): " << t_stat_g << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " allocated node unzip time (max) (s): " << t_stat_g << std::endl;
        ...
        if (!rank)
            std::cout << "a2a_ 2 (nodal) sz  min mean max : " << stat_sz[0] << ", " << stat_sz[1] << ", " << stat_sz[2] << std::endl;
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << "recv sm time  max (s): " << t_stat_g << std::endl;
...
//3.a par sort of local bdy nodes.
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " par::sort local_bdy nodes (max) (s): " << t_stat_g << std::endl;
#endif
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " R1 all2all (max) (s): " << t_stat_g << std::endl;
#endif
...
// all the recv nodes should be found in localBdy
        ...
        m_uiMaxDepth++;
        if (missingKeys.size())
            treeNodesTovtk(missingKeys, rank, "missingKeys");
        m_uiMaxDepth--;
#endif
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " R2 all2all max (s): " << t_stat_g << std::endl;
#endif
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " send sm setup max (s): " << t_stat_g << std::endl;
#endif
        ...
        par::Mpi_Reduce(&t_stat, &t_stat_g, 1, MPI_MAX, 0, comm);
        if (!rank)
            std::cout << " final all2all max (s): " << t_stat_g << std::endl;
#endif
    ...
    void Mesh::performBlocksSetup(unsigned int cLev, unsigned int *tag, unsigned int tsz)
    {
        ...
        for (unsigned int e = 0; e < m_uiLocalBlockList.size(); e++)
        {
            ...
            for (unsigned int i = 0; i < blkKeys.size(); i++)
            {
                ...
                {
                    std::cout << RED << "block diagonal key not found" << NRM << std::endl;
                }
    ...
    ot::Mesh *Mesh::ReMesh(unsigned int grainSz, double ld_tol, unsigned int sfK, unsigned int (*getWeight)(const ot::TreeNode *), unsigned int *blk_tags, unsigned int blk_tag_sz)
    {
        ...
        if (m_uiIsActive)
        {
            ...
            else
            {
                ...
                if (balOct1.size())
                {
                    while ((pCount < m_uiActiveNpes) && (!unBalOctSplitters[pCount].isAncestor(balOct1.front())) && (balOct1.front() > unBalOctSplitters[pCount]))
                        pCount++;
                    ...
                    for (unsigned int e = 0; ((e < balOct1.size()) && (pCount < m_uiActiveNpes)); e++)
                    {
                        ...
                        {
                            while (((e + 1) < balOct1.size()) && (unBalOctSplitters[pCount].isAncestor(balOct1[e + 1])))
                                e++;
                            searchResultIndex[pCount] = e + 1;
                            pCount++;
                        }
    ...
    void Mesh::interGridTransferSendRecvCompute(const ot::Mesh *pMesh)
    {
        ...
        if (m_uiIsActive)
        {
            ...
            for (unsigned int p = 0; p < npes2; p++)
            {
                ...
                if (m2primeSK[sBegin].getOwner() >= 0)
                    m_uiIGTSendC[selectedRank]++;
                if (m2primeSK[sEnd].getOwner() >= 0)
                    m_uiIGTSendC[selectedRank]++;
            }
