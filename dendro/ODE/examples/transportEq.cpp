...
int main(int argc, char **argv)
{
    ...
    {
        if (!rank)
            std::cout << "Usage: " << argv[0] << " maxDepth wavelet_tol partition_tol eleOrder fPrefix=[sol_step] begin=[0] end=[10] th=[0.01] restore_step[=0] restore_rk[=0]" << std::endl;
        return 0;
    }
    ...
    if (argc > 8)
        th = atof(argv[8]);
    if (argc > 9)
        restore_step = atoi(argv[9]);
    if (argc > 10)
        restore_rk = (bool)(atoi(argv[10]));
    ...
    std::function<double(double, double, double, double)> f = [domain_min, domain_max](const double x, const double y, const double z, const double t)
    { return 0; };
    ...
    if (!rank)
        std::cout << "mesh generation ended " << std::endl;
