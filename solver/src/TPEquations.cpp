...
BY_KKofxyz(CCTK_REAL x, CCTK_REAL y, CCTK_REAL z)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        { /* Bowen-York-Curvature :*/
            Aij =
                +1.5 * (TPID::par_P_plus[i] * n_plus[j] + TPID::par_P_plus[j] * n_plus[i] + np_Pp * n_plus[i] * n_plus[j]) / r2_plus + 1.5 * (TPID::par_P_minus[i] * n_minus[j] + TPID::par_P_minus[j] * n_minus[i] + nm_Pm * n_minus[i] * n_minus[j]) / r2_minus - 3.0 * (np_Sp[i] * n_plus[j] + np_Sp[j] * n_plus[i]) / r3_plus - 3.0 * (nm_Sm[i] * n_minus[j] + nm_Sm[j] * n_minus[i]) / r3_minus;
            if (i == j)
