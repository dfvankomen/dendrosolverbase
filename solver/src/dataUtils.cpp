...
void extractBHCoords(const ot::Mesh *pMesh, const DendroScalar *var,
    ...
    if ((pMesh->isActive())) {
        ...
        double v_min =
            vecMin((DendroScalar *)(var + pMesh->getNodeLocalBegin()),
...
bool isRemeshBH(ot::Mesh *pMesh, const Point *bhLoc) {
    const double r_near[2] = {dsolve::DENDROSOLVER_BH1_AMR_R,
    ...
    const unsigned int refLevMin = std::min(dsolve::DENDROSOLVER_BH1_MAX_LEV,
    ...
    if (pMesh->isActive()) {
        ...
        for (unsigned int ele = eleLocalBegin; ele < eleLocalEnd; ele++) {
            ...
            const bool isNearFarTobh1 =
                ((rd1 > r_near[0]) && (rd1 <= r_far[0]));
            const bool isNearFarTobh2 =
                ...
                if (isNearTobh1 || isNearTobh2) {
                    if (isNearTobh1) {
                        if ((pNodes[ele].getLevel() + MAXDEAPTH_LEVEL_DIFF +
                             1) < dsolve::DENDROSOLVER_BH1_MAX_LEV)
                            refine_flags[ele - eleLocalBegin] = OCT_SPLIT;
                        ...
                        if ((pNodes[ele].getLevel() + MAXDEAPTH_LEVEL_DIFF +
                             1) < dsolve::DENDROSOLVER_BH2_MAX_LEV)
                            refine_flags[ele - eleLocalBegin] = OCT_SPLIT;
                        else if ((pNodes[ele].getLevel() +
                                  MAXDEAPTH_LEVEL_DIFF + 1) >
                                 dsolve::DENDROSOLVER_BH2_MAX_LEV)
...
// refinement on the GW when the BH gets closer.
            ...
            if (dBH < 0.1) {
                const unsigned int L_MIN =
                    std::max(2, (int)dsolve::DENDROSOLVER_MAXDEPTH - 4);
                const double dr = temp.abs();

                for (unsigned int i = 0; i < GW::DENDROSOLVER_GW_NUM_RADAII;
                     i++) {
    ...
    if (pMesh->isActive()) {
        ...
        for (unsigned int ele = eleLocalBegin; ele < eleLocalEnd; ele++) {
            refine_flags[ele - eleLocalBegin] =
                (pNodes[ele].getFlag() >> NUM_LEVEL_BITS);
            // std::cout<<"ref flag:
                ...
                for (unsigned int k = 0; k < (eOrder + 1); k++)
                    for (unsigned int j = 0; j < (eOrder + 1); j++)
                        for (unsigned int i = 0; i < (eOrder + 1); i++) {
                            ...
                            // std::cout<<"d1: "<<d1 <<
                            // "BHLOC_0:"<<DENDROSOLVER_BH_LOC[0]<<std::endl;
                            ...
                            if (dBH < 0.1) {
                                if (isNearTobh1 || isNearTobh2) {
                                    // std::cout<<"d1:
                                    // "<<d1.abs()<<"BHLOC_0:"<<DENDROSOLVER_BH_LOC[0]<<std::endl;
                                    // std::cout<<"d2:
                                    // "<<d2.abs()<<"BHLOC_1:"<<DENDROSOLVER_BH_LOC[1]<<std::endl;
                                ...
                                if (isNearTobh1 || isNearTobh2) {
                                    if (isNearTobh1) {
                                        // std::cout<<"d1:
                                        // "<<d1.abs()<<"BHLOC_0:"<<DENDROSOLVER_BH_LOC[0]<<"
                                        ...
                                        // std::cout<<"d2:
                                        // "<<d2.abs()<<"BHLOC_1:"<<DENDROSOLVER_BH_LOC[1]<<"
                                        // rnear: "<<r_near[1]<<std::endl;
