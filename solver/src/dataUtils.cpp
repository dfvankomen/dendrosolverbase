...
namespace dsolve
{
    ...
    void writeBHCoordinates(const ot::Mesh *pMesh, const Point *ptLocs, unsigned int numPt, unsigned int timestep, double time)
    {
        ...
        if (!rankGlobal)
        {
            ...
            std::ofstream fileGW;
            char fName[256];
            sprintf(fName, "%s_BHLocations.dat", dsolve::DENDROSOLVER_PROFILE_FILE_PREFIX.c_str());
    ...
    bool isRemeshBH(ot::Mesh *pMesh, const Point *bhLoc)
    {
        ...
        const double r_near[2] = {dsolve::DENDROSOLVER_BH1_AMR_R, dsolve::DENDROSOLVER_BH2_AMR_R};
        ...
        const unsigned int refLevMin = std::min(dsolve::DENDROSOLVER_BH1_MAX_LEV, dsolve::DENDROSOLVER_BH2_MAX_LEV);
            ...
            for (unsigned int ele = eleLocalBegin; ele < eleLocalEnd; ele++)
            {
                ...
                else
                {
                    ...
                    if (isNearTobh1 || isNearTobh2)
                    {
                        if (isNearTobh1)
                        {
                            if ((pNodes[ele].getLevel() + MAXDEAPTH_LEVEL_DIFF + 1) < dsolve::DENDROSOLVER_BH1_MAX_LEV)
                                refine_flags[ele - eleLocalBegin] = OCT_SPLIT;
                            else if ((pNodes[ele].getLevel() + MAXDEAPTH_LEVEL_DIFF + 1) > dsolve::DENDROSOLVER_BH1_MAX_LEV)
                                refine_flags[ele - eleLocalBegin] = OCT_COARSE;
                            else
                                refine_flags[ele - eleLocalBegin] = OCT_NO_CHANGE;
                        }
                        ...
                        {
                            if ((pNodes[ele].getLevel() + MAXDEAPTH_LEVEL_DIFF + 1) < dsolve::DENDROSOLVER_BH2_MAX_LEV)
                                refine_flags[ele - eleLocalBegin] = OCT_SPLIT;
                            else if ((pNodes[ele].getLevel() + MAXDEAPTH_LEVEL_DIFF + 1) > dsolve::DENDROSOLVER_BH2_MAX_LEV)
                                refine_flags[ele - eleLocalBegin] = OCT_COARSE;
                            else
                                refine_flags[ele - eleLocalBegin] = OCT_NO_CHANGE;
                        }
...
// refinement on the GW when the BH gets closer.
                ...
                if (dBH < 0.1)
                {
                    const unsigned int L_MIN = std::max(2, (int)dsolve::DENDROSOLVER_MAXDEPTH - 4);
                    const double dr = temp.abs();

                    for (unsigned int i = 0; i < GW::DENDROSOLVER_GW_NUM_RADAII; i++)
                    {
                        if (fabs(dr - GW::DENDROSOLVER_GW_RADAII[i]) < 1)
                        {
    ...
    {
        if (!(pMesh->isReMeshUnzip((const double **)unzippedVec, varIds, numVars, wavelet_tol, dsolve::DENDROSOLVER_DENDRO_AMR_FAC)))
            return false;
        ...
        const unsigned int eOrder = pMesh->getElementOrder();
        const double dBH = (DENDROSOLVER_BH_LOC[0] - DENDROSOLVER_BH_LOC[1]).abs();
        const unsigned int refLevMin = std::min(dsolve::DENDROSOLVER_BH1_MAX_LEV, dsolve::DENDROSOLVER_BH2_MAX_LEV);
        ...
        if (pMesh->isActive())
        {
            ...
            for (unsigned int ele = eleLocalBegin; ele < eleLocalEnd; ele++)
            {
                ...
                d1 = temp - DENDROSOLVER_BH_LOC[0];
                d2 = temp - DENDROSOLVER_BH_LOC[1];

                //@milinda: 11/21/2020 : Don't allow to violate the min depth
                if (pNodes[ele].getLevel() < dsolve::DENDROSOLVER_MINDEPTH)
                {
                    refine_flags[ele - eleLocalBegin] = OCT_SPLIT;
                }
                else if (pNodes[ele].getLevel() == dsolve::DENDROSOLVER_MINDEPTH && refine_flags[ele - eleLocalBegin] == OCT_COARSE)
                {
                    refine_flags[ele - eleLocalBegin] = OCT_NO_CHANGE;
                }
                    ...
                    for (unsigned int k = 0; k < (eOrder + 1); k++)
                        for (unsigned int j = 0; j < (eOrder + 1); j++)
                            for (unsigned int i = 0; i < (eOrder + 1); i++)
                            {
                                ...
                                //std::cout<<"d1: "<<d1 << "BHLOC_0:"<<DENDROSOLVER_BH_LOC[0]<<std::endl;
                                ...
                                if (dBH < 0.1)
                                {
                                    if (isNearTobh1 || isNearTobh2)
                                    {
                                        // std::cout<<"d1: "<<d1.abs()<<"BHLOC_0:"<<DENDROSOLVER_BH_LOC[0]<<std::endl;
                                        // std::cout<<"d2: "<<d2.abs()<<"BHLOC_1:"<<DENDROSOLVER_BH_LOC[1]<<std::endl;
                                ...
                                else
                                {
                                    ...
                                    if (isNearTobh1 || isNearTobh2)
                                    {
                                        if (isNearTobh1)
                                        {
                                            //std::cout<<"d1: "<<d1.abs()<<"BHLOC_0:"<<DENDROSOLVER_BH_LOC[0]<<" rnear: "<<r_near[0]<<std::endl;
                                        ...
                                        {
                                            //std::cout<<"d2: "<<d2.abs()<<"BHLOC_1:"<<DENDROSOLVER_BH_LOC[1]<<" rnear: "<<r_near[1]<<std::endl;
