...
RK_SOLVER::RK_SOLVER(ot::Mesh *pMesh, DendroScalar pTBegin, DendroScalar pTEnd,
    ...
    m_uiConstraintVars =
        new DendroScalar *[dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS];
    for (unsigned int index = 0;
         index < dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS; index++)
    ...
    for (unsigned int index = 0; index < dsolve::DENDROSOLVER_ASYNC_COMM_K;
         index++) {
...
void RK_SOLVER::initialGridConverge() {
    ...
    do {
        ...
        if (dsolve::DENDROSOLVER_ENABLE_BLOCK_ADAPTIVITY)
            ...
            if (dsolve::DENDROSOLVER_REFINEMENT_MODE ==
                ...
                isRefine = dsolve::isRemeshEH(
                    ...
                    dsolve::VAR::U_ALPHA, dsolve::DENDROSOLVER_EH_REFINE_VAL,
        ...
        if (isRefine) {
            ot::Mesh *newMesh =
                m_uiMesh->ReMesh(dsolve::DENDROSOLVER_DENDRO_GRAIN_SZ,
...
void RK_SOLVER::writeToVTU(DendroScalar **evolZipVarIn,
    ...
    for (unsigned int i = 0; i < numEvolVars; i++) {
        pDataNames.push_back(
            std::string(dsolve::DENDROSOLVER_VAR_NAMES[evolVarIndices[i]]));
        pData[i] = evolZipVarIn[evolVarIndices[i]];
    }

    for (unsigned int i = 0; i < numConstVars; i++) {
        pDataNames.push_back(std::string(
            dsolve::DENDROSOLVER_VAR_CONSTRAINT_NAMES[constVarIndices[i]]));
...
void RK_SOLVER::unzipVars_async(DendroScalar **zipIn, DendroScalar **uzipOut) {
    ...
    for (unsigned int var = 0; var < dsolve::DENDROSOLVER_NUM_VARS;
         var += dsolve::DENDROSOLVER_ASYNC_COMM_K) {
...
void RK_SOLVER::rkSolve() {
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % dsolve::DENDROSOLVER_CHECKPT_FREQ) == 0)
        ...
        if ((m_uiMesh->isActive()) &&
            ...
            dsolve::timer::profileInfoIntermediate(
                dsolve::DENDROSOLVER_PROFILE_FILE_PREFIX.c_str(), m_uiMesh,
                m_uiCurrentStep);
        }
        ...
        if ((m_uiCurrentStep % dsolve::DENDROSOLVER_REMESH_TEST_FREQ) == 0) {
            ...
            // **)m_uiPrevVar,refineVarIds,refineNumVars,dsolve::DENDROSOLVER_WAVELET_TOL);
            unzipVars(m_uiPrevVar, m_uiUnzipVar);
#endif
            ...
            if (isRefine) {
                ...
                // deallocate constraint vars allocate them for the new mesh.
                for (unsigned int index = 0;
                     index < dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS;
                     index++) {
                    delete[] m_uiConstraintVars[index];
                    delete[] m_uiUnzipConstraintVars[index];
    ...
    double consVecMax[dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS];*/
            ...
            if ((m_uiCurrentStep % dsolve::DENDROSOLVER_IO_OUTPUT_FREQ) == 0)
                writeToVTU(m_uiPrevVar, m_uiConstraintVars,
                           ...
                           dsolve::DENDROSOLVER_VTU_OUTPUT_CONST_INDICES,
...
void RK_SOLVER::storeCheckPoint(const char *fNamePrefix) {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiCurrentStep % (2 * dsolve::DENDROSOLVER_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
...
void RK_SOLVER::restoreCheckPoint(const char *fNamePrefix, MPI_Comm comm) {
    ...
    for (unsigned int cpIndex = restoreFileIndex; cpIndex < 2; cpIndex++) {
        ...
        newMesh->setDomainBounds(Point(dsolve::DENDROSOLVER_GRID_MIN_X,
                                       dsolve::DENDROSOLVER_GRID_MIN_Y,
                                       dsolve::DENDROSOLVER_GRID_MIN_Z),
                                 Point(dsolve::DENDROSOLVER_GRID_MAX_X,
            ...
            /*for(unsigned int i=0;i<numVars;i++)
    {
        ...
        if(restoreStatus==1) break;
            ...
            restoreStatus = io::checkpoint::readVecFromFile(
                fName, newMesh, m_uiPrevVar, dsolve::DENDROSOLVER_NUM_VARS);
        }
