...
namespace ode
{
    namespace solver
    {
        ...
        RK_SOLVER::RK_SOLVER(ot::Mesh *pMesh, DendroScalar pTBegin, DendroScalar pTEnd, DendroScalar pTh, RKType rkType) : RK(pMesh, pTBegin, pTEnd, pTh)
        {
            ...
            m_uiVar = new DendroScalar *[dsolve::DENDROSOLVER_NUM_VARS];
            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                m_uiVar[index] = m_uiMesh->createVector<DendroScalar>();

            m_uiPrevVar = new DendroScalar *[dsolve::DENDROSOLVER_NUM_VARS];
            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                m_uiPrevVar[index] = m_uiMesh->createVector<DendroScalar>();

            m_uiVarIm = new DendroScalar *[dsolve::DENDROSOLVER_NUM_VARS];
            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                m_uiVarIm[index] = m_uiMesh->createVector<DendroScalar>();

            if (m_uiRKType == RKType::RK3)
                m_uiNumRKStages = dsolve::DENDROSOLVER_RK3_STAGES;
            else if (m_uiRKType == RKType::RK4)
                m_uiNumRKStages = dsolve::DENDROSOLVER_RK4_STAGES;
            else if (m_uiRKType == RKType::RK45)
                m_uiNumRKStages = dsolve::DENDROSOLVER_RK45_STAGES;
            else
            {
            ...
            {
                m_uiStage[stage] = new DendroScalar *[dsolve::DENDROSOLVER_NUM_VARS];
                for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                    m_uiStage[stage][index] = m_uiMesh->createVector<DendroScalar>();
            }

            m_uiUnzipVar = new DendroScalar *[dsolve::DENDROSOLVER_NUM_VARS];
            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                m_uiUnzipVar[index] = m_uiMesh->createUnZippedVector<DendroScalar>();

            m_uiUnzipVarRHS = new DendroScalar *[dsolve::DENDROSOLVER_NUM_VARS];
            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                m_uiUnzipVarRHS[index] = m_uiMesh->createUnZippedVector<DendroScalar>();

            // allocate memory for the constraint variables.
            m_uiConstraintVars = new DendroScalar *[dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS];
            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS; index++)
                m_uiConstraintVars[index] = m_uiMesh->createVector<DendroScalar>();

            m_uiUnzipConstraintVars = new DendroScalar *[dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS];
            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS; index++)
                m_uiUnzipConstraintVars[index] = m_uiMesh->createUnZippedVector<DendroScalar>();

            // mpi communication
            m_uiSendNodeBuf = new DendroScalar *[dsolve::DENDROSOLVER_ASYNC_COMM_K];
            m_uiRecvNodeBuf = new DendroScalar *[dsolve::DENDROSOLVER_ASYNC_COMM_K];

            m_uiSendReqs = new MPI_Request *[dsolve::DENDROSOLVER_ASYNC_COMM_K];
            m_uiRecvReqs = new MPI_Request *[dsolve::DENDROSOLVER_ASYNC_COMM_K];
            m_uiSendSts = new MPI_Status *[dsolve::DENDROSOLVER_ASYNC_COMM_K];
            m_uiRecvSts = new MPI_Status *[dsolve::DENDROSOLVER_ASYNC_COMM_K];

            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_ASYNC_COMM_K; index++)
            {
            ...
            if (m_uiMesh->isActive())
            {
                ...
                for (unsigned int index = 0; index < dsolve::DENDROSOLVER_ASYNC_COMM_K; index++)
                {
                    if (m_uiMesh->getGhostExcgTotalSendNodeCount() != 0)
                        m_uiSendNodeBuf[index] = new DendroScalar[m_uiMesh->getGhostExcgTotalSendNodeCount()];
                    if (m_uiMesh->getGhostExcgTotalRecvNodeCount() != 0)
                        m_uiRecvNodeBuf[index] = new DendroScalar[m_uiMesh->getGhostExcgTotalRecvNodeCount()];
        ...
        void RK_SOLVER::applyInitialConditions(DendroScalar **zipIn)
        {
            ...
            double *var = new double[dsolve::DENDROSOLVER_NUM_VARS];
        ...
        void RK_SOLVER::initialGridConverge()
        {
            ...
            const unsigned int refineNumVars = dsolve::DENDROSOLVER_NUM_REFINE_VARS;
            ...
            do
            {
                ...
                else
                {
                    ...
                    {
                        //isRefine=m_uiMesh->isReMeshUnzip((const double **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,dsolve::DENDROSOLVER_DENDRO_AMR_FAC);
                        isRefine = dsolve::isReMeshWAMR(m_uiMesh, (const double **)m_uiUnzipVar, refineVarIds, refineNumVars, waveletTolFunc, dsolve::DENDROSOLVER_DENDRO_AMR_FAC);
                    }
                    else if (dsolve::DENDROSOLVER_REFINEMENT_MODE == dsolve::RefinementMode::EH)
                    {
                        isRefine = dsolve::isRemeshEH(m_uiMesh, (const double **)m_uiUnzipVar, dsolve::VAR::U_ALPHA, dsolve::DENDROSOLVER_EH_REFINE_VAL, dsolve::DENDROSOLVER_EH_COARSEN_VAL, true);
                    }
                    else if (dsolve::DENDROSOLVER_REFINEMENT_MODE == dsolve::RefinementMode::EH_WAMR)
                    {
                        const bool isR1 = dsolve::isReMeshWAMR(m_uiMesh, (const double **)m_uiUnzipVar, refineVarIds, refineNumVars, waveletTolFunc, dsolve::DENDROSOLVER_DENDRO_AMR_FAC);
                        const bool isR2 = dsolve::isRemeshEH(m_uiMesh, (const double **)m_uiUnzipVar, dsolve::VAR::U_ALPHA, dsolve::DENDROSOLVER_EH_REFINE_VAL, dsolve::DENDROSOLVER_EH_COARSEN_VAL, false);
                ...
                if (isRefine)
                {
                    ot::Mesh *newMesh = m_uiMesh->ReMesh(dsolve::DENDROSOLVER_DENDRO_GRAIN_SZ, dsolve::DENDROSOLVER_LOAD_IMB_TOL, dsolve::DENDROSOLVER_SPLIT_FIX);
                    ...
                    if (!(m_uiMesh->getMPIRankGlobal()))
                        std::cout << "initial grid iteration : " << iterCount << " old mesh (ele): " << oldElements_g << " new mesh(ele): " << newElements_g << std::endl;
                    if (!(m_uiMesh->getMPIRankGlobal()))
                        std::cout << "initial grid iteration : " << iterCount << " old mesh (zip nodes): " << oldGridPoints_g << " new mesh(zip nodes): " << newGridPoints_g << std::endl;
                    ...
                    if (m_uiMesh->isActive())
                    {
                        DendroScalar l_min = vecMin(m_uiPrevVar[dsolve::VAR::U_ALPHA] + m_uiMesh->getNodeLocalBegin(), (m_uiMesh->getNumLocalMeshNodes()), m_uiMesh->getMPICommunicator());
                        DendroScalar l_max = vecMax(m_uiPrevVar[dsolve::VAR::U_ALPHA] + m_uiMesh->getNodeLocalBegin(), (m_uiMesh->getNumLocalMeshNodes()), m_uiMesh->getMPICommunicator());
            ...
            m_uiT_h = dsolve::DENDROSOLVER_RK45_TIME_STEP_SIZE;
            if (!m_uiMesh->getMPIRankGlobal())
            {
        ...
        void RK_SOLVER::writeToVTU(DendroScalar **evolZipVarIn, DendroScalar **constrZipVarIn, unsigned int numEvolVars, unsigned int numConstVars, const unsigned int *evolVarIndices, const unsigned int *constVarIndices, bool zslice)
        {
            ...
            {
                pDataNames.push_back(std::string(dsolve::DENDROSOLVER_VAR_NAMES[evolVarIndices[i]]));
                pData[i] = evolZipVarIn[evolVarIndices[i]];
            }

            for (unsigned int i = 0; i < numConstVars; i++)
            {
                pDataNames.push_back(std::string(dsolve::DENDROSOLVER_VAR_CONSTRAINT_NAMES[constVarIndices[i]]));
            ...
            char fPrefix[256];
            sprintf(fPrefix, "%s_%d", dsolve::DENDROSOLVER_VTU_FILE_PREFIX.c_str(), m_uiCurrentStep);
        ...
        void RK_SOLVER::intergridTransferVars(DendroScalar **&zipIn, const ot::Mesh *pnewMesh)
        {
        ...
        void RK_SOLVER::unzipVars_async(DendroScalar **zipIn, DendroScalar **uzipOut)
        {
            ...
            for (unsigned int var = 0; var < dsolve::DENDROSOLVER_NUM_VARS; var += dsolve::DENDROSOLVER_ASYNC_COMM_K)
            {

                for (unsigned int i = 0; (i < dsolve::DENDROSOLVER_ASYNC_COMM_K); i++)
        ...
        void RK_SOLVER::performSingleIteration()
        {
            ...
            io::varToRawData((const ot::Mesh*)m_uiMesh,(const double **)m_uiPrevVar,dsolve::DENDROSOLVER_NUM_VARS,NULL,frawName);
                ...
                if (m_uiRKType == RKType::RK3)
                {
                    ...
                    dendroSolverRHS(m_uiUnzipVarRHS, (const DendroScalar **)m_uiUnzipVar, &(*(blkList.begin())), blkList.size());
            ...
            io::varToRawData((const ot::Mesh*)m_uiMesh,(const double **)m_uiStage[0],dsolve::DENDROSOLVER_NUM_VARS,NULL,frawName);
#endif
            ...
            io::varToRawData((const ot::Mesh*)m_uiMesh,(const double **)m_uiStage[1],dsolve::DENDROSOLVER_NUM_VARS,NULL,frawName);
#endif
            ...
            #if 0
            sprintf(frawName,"rkU_%d",3*m_uiCurrentStep+(1));
            io::varToRawData((const ot::Mesh*)m_uiMesh,(const double **)m_uiVarIm,dsolve::DENDROSOLVER_NUM_VARS,NULL,frawName);
            #endif            
         ...
         */
                ...
                { // rk4 solver
                    ...
                    for (unsigned int stage = 0; stage < (dsolve::DENDROSOLVER_RK4_STAGES - 1); stage++)
                    {
                        ...
                        cuda::computeRHS(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, &(*(blkList.begin())), blkList.size(), (const cuda::EMDAComputeParams *)&dsolveParams, threadBlock, pt_min, pt_max, 1);
#else
                        ...
                        for (unsigned int blk = 0; blk < blkList.size(); blk++)
                        {
                            ...
                            dendroSolverRHSUnpacked_sep(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, offset, ptmin, ptmax, sz, bflag);
#else
                            dendroSolverRHSUnpacked(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, offset, ptmin, ptmax, sz, bflag);
#endif
                        }
#endif
...
#ifdef DEBUG_RK_SOLVER
                        if (!rank)
                            std::cout << " stage: " << stage << " af rhs UNZIP RHS TEST:" << std::endl;
                        for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                    ...
                    current_t_adv = current_t + RK4_T[(dsolve::DENDROSOLVER_RK4_STAGES - 1)] * m_uiT_h;

#ifdef DEBUG_RK_SOLVER
                    if (!rank)
                        std::cout << " stage: " << (dsolve::DENDROSOLVER_RK4_STAGES - 1) << " begin: " << std::endl;
...
#ifdef DEBUG_RK_SOLVER
                    if (!rank)
                        std::cout << " stage: " << (dsolve::DENDROSOLVER_RK4_STAGES - 1) << " af rhs UNZIP RHS TEST:" << std::endl;
                    for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                        ot::test::isUnzipInternalNaN(m_uiMesh, m_uiUnzipVarRHS[index]);
#endif
                    ...
                    for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd; node++)
                    {
                        ...
                        for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                        {
                            ...
                            for (unsigned int s = 0; s < (dsolve::DENDROSOLVER_RK4_STAGES); s++)
                            {
                ...
                { // rk45 solver
                    ...
                    do
                    {
                        ...
                        if (m_uiMesh->isActive())
                        {
                            ...
                            for (unsigned int stage = 0; stage < (dsolve::DENDROSOLVER_RK45_STAGES - 1); stage++)
                            {
                            ...
                            current_t_adv = current_t + RK_T[(dsolve::DENDROSOLVER_RK45_STAGES - 1)];

#ifdef DEBUG_RK_SOLVER
                            if (!rank)
                                std::cout << " stage: " << (dsolve::DENDROSOLVER_RK45_STAGES - 1) << " begin: " << std::endl;
...
#ifdef DEBUG_RK_SOLVER
                            if (!rank)
                                std::cout << " stage: " << (dsolve::DENDROSOLVER_RK45_STAGES - 1) << " af rhs UNZIP RHS TEST:" << std::endl;

                            for (unsigned int index = 0; index < dsolve::DENDROSOLVER_NUM_VARS; index++)
                                ot::test::isUnzipInternalNaN(m_uiMesh, m_uiUnzipVarRHS[index]);
#endif

                            zipVars(m_uiUnzipVarRHS, m_uiStage[(dsolve::DENDROSOLVER_RK45_STAGES - 1)]);
                        ...
                        for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                            ...
                            for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd; node++)
                            {
                                ...
                                {
                                    m_uiVarIm[index][node] = m_uiPrevVar[index][node];
                                    for (unsigned int s = 0; s < (dsolve::DENDROSOLVER_RK45_STAGES - 1); s++)
                                    {
                                        if (s == 1)
                                            continue;
                                    ...
                                    m_uiVar[index][node] = m_uiPrevVar[index][node];
                                    for (unsigned int s = 0; s < dsolve::DENDROSOLVER_RK45_STAGES; s++)
                                    {
                                        if (s == 1)
                        ...
                        {
                            repeatStep = true;
                            m_uiT_h = dsolve::DENDROSOLVER_SAFETY_FAC * m_uiT_h * (pow(fabs(dsolve::DENDROSOLVER_RK45_DESIRED_TOL / n_inf_max), 0.25));
                            if (!m_uiMesh->getMPIRankGlobal())
                                std::cout << " repeat : " << m_uiCurrentStep << " with : " << m_uiT_h << std::endl;
                        }
                        else
                        {
                            repeatStep = false;
                            m_uiT_h = dsolve::DENDROSOLVER_SAFETY_FAC * m_uiT_h * (pow(fabs(dsolve::DENDROSOLVER_RK45_DESIRED_TOL / n_inf_max), 0.20));
                        }
        ...
        if(m_uiMesh->getMPICommSizeGlobal()==1)
        {
            ...
            grid_limits[0] = Point(dsolve::DENDROSOLVER_OCTREE_MIN[0], dsolve::DENDROSOLVER_OCTREE_MIN[1], dsolve::DENDROSOLVER_OCTREE_MIN[2]);
            grid_limits[1] = Point(dsolve::DENDROSOLVER_OCTREE_MAX[0], dsolve::DENDROSOLVER_OCTREE_MAX[1], dsolve::DENDROSOLVER_OCTREE_MAX[2]);

            domain_limits[0] = Point(dsolve::DENDROSOLVER_COMPD_MIN[0], dsolve::DENDROSOLVER_COMPD_MIN[1], dsolve::DENDROSOLVER_COMPD_MIN[2]);
            domain_limits[1] = Point(dsolve::DENDROSOLVER_COMPD_MAX[0], dsolve::DENDROSOLVER_COMPD_MAX[1], dsolve::DENDROSOLVER_COMPD_MAX[2]);
            ...
            double var[dsolve::DENDROSOLVER_NUM_VARS];
            double mp, mm, mp_adm, mm_adm, E, J1, J2, J3;


            for(unsigned int j=0; j < nx; j++)
            {
                double x = dsolve::BH1.getBHCoordX();
                double y = domain_limits[0].y() +j*hy;
                double z = dsolve::BH1.getBHCoordZ();
                ...
                alpha[j] = var[dsolve::VAR::U_ALPHA];
                chi[j] = var[dsolve::VAR::U_CHI];
                at11[j] = var[dsolve::VAR::U_SYMAT0];
                at12[j] = var[dsolve::VAR::U_SYMAT1];
                at13[j] = var[dsolve::VAR::U_SYMAT2];
                at22[j] = var[dsolve::VAR::U_SYMAT3];
                at23[j] = var[dsolve::VAR::U_SYMAT4];
                at33[j] = var[dsolve::VAR::U_SYMAT5];

                gt11[j] = var[dsolve::VAR::U_SYMGT0];
                gt12[j] = var[dsolve::VAR::U_SYMGT1];
                gt13[j] = var[dsolve::VAR::U_SYMGT2];
                gt22[j] = var[dsolve::VAR::U_SYMGT3];
                gt23[j] = var[dsolve::VAR::U_SYMGT4];
                gt33[j] = var[dsolve::VAR::U_SYMGT5];
            }

            ot::da::interpolateToCoords(m_uiMesh,m_uiPrevVar[dsolve::VAR::U_SYMAT0],&(*(coords.begin())),coords.size(), grid_limits, domain_limits , &(*(interp.begin())),validIndex);
            std::cout<<" nx: "<<nx<<" validIndex size: "<<validIndex.size();
            assert(validIndex.size() == nx);

            std::ofstream outf;
            char fName[200];
            sprintf(fName,"%s_lo.csv",dsolve::DENDROSOLVER_VAR_NAMES[dsolve::VAR::U_SYMAT0]);
            outf.open(fName);
            ...
            for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h)
            {
                ...
                if (is_merged)
                {
                    dsolve::DENDROSOLVER_REMESH_TEST_FREQ = dsolve::DENDROSOLVER_REMESH_TEST_FREQ_AFTER_MERGER;
                    dsolve::DENDROSOLVER_GW_EXTRACT_FREQ = dsolve::DENDROSOLVER_GW_EXTRACT_FREQ_AFTER_MERGER;
                }

                // checkpoint the previous solution value before going to the next step.
                dsolve::timer::t_ioCheckPoint.start();
                if ((m_uiMesh->isActive()) && (m_uiCurrentStep % dsolve::DENDROSOLVER_CHECKPT_FREQ) == 0)
                    storeCheckPoint(dsolve::DENDROSOLVER_CHKPT_FILE_PREFIX.c_str());
                dsolve::timer::t_ioCheckPoint.stop();

                // write sol to vtu.
                if ((m_uiMesh->isActive()) && (m_uiCurrentStep % dsolve::DENDROSOLVER_TIME_STEP_OUTPUT_FREQ) == 0)
                {
                    // TODO: could generate calculating and printting the l_min and l_max for literally all of the variables
                    l_min = vecMin(m_uiPrevVar[dsolve::VAR::U_ALPHA] + m_uiMesh->getNodeLocalBegin(), (m_uiMesh->getNumLocalMeshNodes()), m_uiMesh->getMPICommunicator());
                    l_max = vecMax(m_uiPrevVar[dsolve::VAR::U_ALPHA] + m_uiMesh->getNodeLocalBegin(), (m_uiMesh->getNumLocalMeshNodes()), m_uiMesh->getMPICommunicator());
                    if (!m_uiMesh->getMPIRank())
                    {
                    ...
                    dsolve::timer::profileInfoIntermediate(dsolve::DENDROSOLVER_PROFILE_FILE_PREFIX.c_str(), m_uiMesh, m_uiCurrentStep);
                }

                if ((m_uiCurrentStep % dsolve::DENDROSOLVER_TIME_STEP_OUTPUT_FREQ) == 0)
                    dsolve::timer::resetSnapshot();

                if ((m_uiCurrentStep % dsolve::DENDROSOLVER_REMESH_TEST_FREQ) == 0)
                {
...
#else
                    performGhostExchangeVars(m_uiPrevVar);
                    //isRefine=m_uiMesh->isReMesh((const double **)m_uiPrevVar,refineVarIds,refineNumVars,dsolve::DENDROSOLVER_WAVELET_TOL);
                    unzipVars(m_uiPrevVar, m_uiUnzipVar);
#endif

                    //                     char fPrefix[256];
                    //                     sprintf(fPrefix,"%s_wavelets_%d",dsolve::DENDROSOLVER_VTU_FILE_PREFIX.c_str(),m_uiCurrentStep);
                    ...
                    else
                    {
                        if (dsolve::DENDROSOLVER_REFINEMENT_MODE == dsolve::RefinementMode::WAMR)
                        {
                            // TEMP: make sure this is removed later, just for debugging now
                            isRefine = true;
                            //isRefine=m_uiMesh->isReMeshUnzip((const double **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,dsolve::DENDROSOLVER_DENDRO_AMR_FAC);
                            // isRefine = dsolve::isReMeshWAMR(m_uiMesh, (const double **)m_uiUnzipVar, refineVarIds, refineNumVars, waveletTolFunc, dsolve::DENDROSOLVER_DENDRO_AMR_FAC);
                        }
                    ...
                    if (isRefine)
                    {
                        ...
                        if (!(m_uiMesh->getMPIRankGlobal()))
                            std::cout << "step : " << m_uiCurrentStep << " time : " << m_uiCurrentTime << " old mesh: " << oldElements_g << " new mesh: " << newElements_g << std::endl;
                ...
                if ((m_uiCurrentStep % dsolve::DENDROSOLVER_GW_EXTRACT_FREQ) == 0)
                {
            ...
            double consVecMax[dsolve::DENDROSOLVER_CONSTRAINT_NUM_VARS];*/
                    ...
                    dsolve::extractConstraints(m_uiMesh, (const DendroScalar **)m_uiConstraintVars, m_uiPrevVar[BHLOC::EXTRACTION_VAR_ID], BHLOC::EXTRACTION_TOL, m_uiCurrentStep, m_uiCurrentTime);
                    ...
                    if ((m_uiCurrentStep % dsolve::DENDROSOLVER_IO_OUTPUT_FREQ) == 0)
                        writeToVTU(m_uiPrevVar, m_uiConstraintVars, dsolve::DENDROSOLVER_NUM_EVOL_VARS_VTU_OUTPUT, dsolve::DENDROSOLVER_NUM_CONST_VARS_VTU_OUTPUT, dsolve::DENDROSOLVER_VTU_OUTPUT_EVOL_INDICES, dsolve::DENDROSOLVER_VTU_OUTPUT_CONST_INDICES, dsolve::DENDROSOLVER_VTU_Z_SLICE_ONLY);
#endif

#ifdef DENDROSOLVER_EXTRACT_BH_LOCATIONS
                    dsolve::writeBHCoordinates((const ot::Mesh *)m_uiMesh, (const Point *)m_uiBHLoc, 2, m_uiCurrentStep, m_uiCurrentTime);
#endif
                }
...
#ifdef DENDROSOLVER_EXTRACT_BH_LOCATIONS
                //dsolve::extractBHCoords((const ot::Mesh *)m_uiMesh,(const DendroScalar*)m_uiVar[BHLOC::EXTRACTION_VAR_ID],BHLOC::EXTRACTION_TOL,(const Point *) m_uiBHLoc,2,(Point*)bhLoc);
                dsolve::computeBHLocations((const ot::Mesh *)m_uiMesh, m_uiBHLoc, bhLoc, m_uiPrevVar, m_uiT_h);
                m_uiBHLoc[0] = bhLoc[0];
                m_uiBHLoc[1] = bhLoc[1];
                ...
                //dsolve::artificial_dissipation(m_uiMesh,m_uiPrevVar,dsolve::DENDROSOLVER_NUM_VARS,dsolve::DENDROSOLVER_DISSIPATION_NC,dsolve::DENDROSOLVER_DISSIPATION_S,false);
        ...
        void RK_SOLVER::storeCheckPoint(const char *fNamePrefix)
        {
            ...
            if (m_uiMesh->isActive())
            {
                ...
                (m_uiCurrentStep % (2 * dsolve::DENDROSOLVER_CHECKPT_FREQ) == 0) ? cpIndex = 0 : cpIndex = 1; // to support alternate file writing.
                unsigned int rank = m_uiMesh->getMPIRank();
                ...
                /*for(unsigned int i=0;i<numVars;i++)
        {
            ...
            io::checkpoint::writeVecToFile(fName,m_uiMesh,m_uiPrevVar[i]);
                ...
                io::checkpoint::writeVecToFile(fName, m_uiMesh, (const double **)m_uiPrevVar, dsolve::DENDROSOLVER_NUM_VARS);
                ...
                if (!rank)
                {
                    ...
                    checkPoint["DENDRO_RK45_WAVELET_TOLERANCE"] = dsolve::DENDROSOLVER_WAVELET_TOL;
                    checkPoint["DENDRO_RK45_LOAD_IMB_TOLERANCE"] = dsolve::DENDROSOLVER_LOAD_IMB_TOL;
        ...
        void RK_SOLVER::restoreCheckPoint(const char *fNamePrefix, MPI_Comm comm)
        {
            ...
            for (unsigned int cpIndex = 0; cpIndex < 2; cpIndex++)
            {
                ...
                if (!rank)
                {
                    ...
                    if (restoreStatus == 0)
                    {
                        ...
                        dsolve::DENDROSOLVER_WAVELET_TOL = checkPoint["DENDRO_RK45_WAVELET_TOLERANCE"];
                        dsolve::DENDROSOLVER_LOAD_IMB_TOL = checkPoint["DENDRO_RK45_LOAD_IMB_TOLERANCE"];
            ...
            for (unsigned int cpIndex = restoreFileIndex; cpIndex < 2; cpIndex++)
            {
                ...
                octree.clear();
                if (!rank)
                    std::cout << " Trying to restore from checkpoint index : " << cpIndex << std::endl;
                ...
                {
                    if (!rank)
                        std::cout << " [Error] : checkpoint file written from  a larger communicator than the current global comm. (i.e. communicator shrinking not allowed in the restore step. )" << std::endl;
                    exit(0);
                }
                ...
                if (restoreStatusGlobal == 1)
                {
                    ...
                    if (!rank)
                        std::cout << "[Error]: octree (*.oct) restore file currupted " << std::endl;
                    continue;
                }
                    ...
                    /*for(unsigned int i=0;i<numVars;i++)
            {
                ...
                if(restoreStatus==1) break;
                    ...
                    restoreStatus = io::checkpoint::readVecFromFile(fName, newMesh, m_uiPrevVar, dsolve::DENDROSOLVER_NUM_VARS);
                }
                ...
                if (restoreStatusGlobal == 1)
                {
                    ...
                    if (!rank)
                        std::cout << "[Error]: varible (*.var) restore file currupted " << std::endl;
                    continue;
                }
            ...
            if (!rank)
                std::cout << " checkpoint at step : " << m_uiCurrentStep << "active Comm. sz: " << activeCommSz << " restore successful: "
