...
// TODO: fix the TWO puncture code for ANYTHING!
  ...
  */
    ...
    {
        vars[VAR::U_ALPHA] =
            ((1.0 - 0.5 * *mp / r_plus - 0.5 * *mm / r_minus) / (1.0 + 0.5 * *mp / r_plus + 0.5 * *mm / r_minus));

        if (r_plus < TPID::TP_Extend_Radius)
        {
            vars[VAR::U_ALPHA] =
                ((1.0 - 0.5 * EXTEND(*mp, r_plus) - 0.5 * *mm / r_minus) / (1.0 + 0.5 * EXTEND(*mp, r_plus) + 0.5 * *mm / r_minus));
        }
        if (r_minus < TPID::TP_Extend_Radius)
        {
            vars[VAR::U_ALPHA] =
                ((1.0 - 0.5 * EXTEND(*mm, r_minus) - 0.5 * *mp / r_plus) / (1.0 + 0.5 * EXTEND(*mp, r_minus) + 0.5 * *mp / r_plus));
        }
