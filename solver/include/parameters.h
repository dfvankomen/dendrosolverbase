...
with open(os.path.join(os.path.dirname(PARAM_SETUP_FILE), "solver_parameters.sample.toml"), "w") as f:
    f.write(dendrosym.params.generate_sample_config_file_text("dsolve", PARAM_SETUP_FILE))
...
namespace dsolve
{
    ...
    static const unsigned int DENDROSOLVER_NUM_VARS_INTENL = (DENDROSOLVER_RK45_STAGES + 1) * DENDROSOLVER_NUM_VARS;
    ...
    /** @brief: Solution output gap (instead of frequency, we can use to output the solution if currentTime > lastIOOutputTime + DENDROSOLVER_IO_OUTPUT_GAP) */
    ...
    /** @brief: Dendro coarsening factor, if computed wavelet tol < DENDROSOLVER_DENDRO_AMR_FAC*DENDROSOLVER_WAVELET_TOL */
    ...
    /** @brief: The Courant factor: CFL stability number (specifies how dt=DENDROSOLVER_CFL_FACTOR*dx) */
    ...
    /** @brief: The dissipation "S", note this is only called in a comment for "artificial dissipation" which appears to not be defined anywhere */
    ...
    /** @brief: Variable group size for the asynchronous unzip operation. This is an async communication. (Upper bound should be DENDROSOLVER_NUM_VARS) */
    ...
    /** @brief: Skip grid point distance from bh < DENDROSOLVER_BH1_CONSTRAINT_R when computing the constraint norms for BH1 */
    ...
    /** @brief: Skip grid point distance from bh < DENDROSOLVER_BH2_CONSTRAINT_R when computing the constraint norms  for BH2 */
