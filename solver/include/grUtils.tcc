                ...
                for (unsigned int j = 0; j < (eleOrder + 1); j++)
                    for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                        ...
                        if ((nodeLookUp_CG >= nodeLocalBegin &&
                            ...
                            Point grid_pt(x, y, z);
                            const double d1 =
                                (grid_pt - dsolve::BSSN_BH_LOC[0]).abs();
                            const double d2 =
                                (grid_pt - dsolve::BSSN_BH_LOC[1]).abs();

                            if ((d1 < dsolve::BSSN_BH1_CONSTRAINT_R ||
                                 d2 < dsolve::BSSN_BH2_CONSTRAINT_R))
    ...
    if (mesh->isActive()) {
        ...
        for (unsigned int index = 0; index < numConstraints; index++) {
            ...
            if (!rankActive) {
                std::cout << YLW << "\tConstraint "
                          << dsolve::BSSN_CONSTRAINT_VAR_NAMES[index]
        ...
        if (!rankActive) {
            ...
            sprintf(fName, "%s_Constraints.dat",
                    dsolve::BSSN_PROFILE_FILE_PREFIX.c_str());
