...
namespace dsolve
{
    ...
    double extractConstraints(const ot::Mesh *mesh, const T **constraintVar, const T *maskVec, double maskthreshoold, unsigned int timestep, double stime)
    {
        ...
        if (mesh->isActive())
        {
            ...
            {
                constraintMaskedL2[index] = dsolve::computeConstraintL2Norm(mesh, constraintVar[index], maskVec, maskthreshoold);
                if (!rankActive)
                {
                    std::cout << YLW << "\tConstraint " << dsolve::DENDROSOLVER_VAR_CONSTRAINT_NAMES[index] << " L2 : (" << constraintMaskedL2[index] << " )" << NRM << std::endl;
                }
            }
            ...
            if (!rankActive)
            {
                ...
                std::ofstream fileGW;
                char fName[256];
                sprintf(fName, "%s_Constraints.dat", dsolve::DENDROSOLVER_PROFILE_FILE_PREFIX.c_str());
