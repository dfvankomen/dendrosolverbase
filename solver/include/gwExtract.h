...
void extractFarFieldPsi4(const ot::Mesh *mesh, const T **cVar,
        ...
        for (unsigned int k = 0; k < DENDROSOLVER_GW_NUM_RADAII; k++) {
            for (unsigned int pts = 0; pts < numPts; pts++) {
                domain_coords[3 * pts + 0] =
                    (DENDROSOLVER_GW_RADAII[k] * sin(LEBEDEV_THETA[pts]) *
                     cos(LEBEDEV_PHI[pts]));
                domain_coords[3 * pts + 1] =
                    (DENDROSOLVER_GW_RADAII[k] * sin(LEBEDEV_THETA[pts]) *
                     sin(LEBEDEV_PHI[pts]));
                domain_coords[3 * pts + 2] =
                    (DENDROSOLVER_GW_RADAII[k] * cos(LEBEDEV_THETA[pts]));
            }
            ...
            for (unsigned int l = 0; l < DENDROSOLVER_GW_NUM_LMODES; l++) {
                for (unsigned int m = 0; m < 2 * DENDROSOLVER_GW_L_MODES[l] + 1;
                     m++) {
    ...
    // for(unsigned int k=0;k<DENDROSOLVER_GW_NUM_RADAII;k++)
    MPI_Reduce(swsh_coeff, swsh_coeff_g,
               (DENDROSOLVER_GW_NUM_RADAII * TOTAL_MODES), MPI_DOUBLE_COMPLEX,
               MPI_SUM, 0, commGlobal);
    MPI_Reduce(&(*(psi4L2R.begin())), &(*(psi4L2R_g.begin())),
               DENDROSOLVER_GW_NUM_RADAII, MPI_DOUBLE, MPI_SUM, 0, commGlobal);
    ...
    if (!rankGlobal) {
        ...
        std::ofstream fileGW;
        char fName[256];
        sprintf(fName, "%s_GW_L2.dat",
                dsolve::DENDROSOLVER_PROFILE_FILE_PREFIX.c_str());
        ...
        for (unsigned int l = 0; l < DENDROSOLVER_GW_NUM_LMODES; l++) {
            for (unsigned int m = 0; m < 2 * DENDROSOLVER_GW_L_MODES[l] + 1;
                ...
                // std::cout<<"l : "<<DENDROSOLVER_GW_L_MODES[l]<< " m :
                // "<<(int)(m-DENDROSOLVER_GW_L_MODES[l])<<std::endl;
                sprintf(fName, "%s_GW_l%d_m%d.dat",
                        dsolve::DENDROSOLVER_PROFILE_FILE_PREFIX.c_str(),
                        DENDROSOLVER_GW_L_MODES[l],
                        (int)(m - DENDROSOLVER_GW_L_MODES[l]));
                fileGW.open(fName, std::ofstream::app);

                if (timestep == 0) {
                    fileGW << "TimeStep\t"
                           << " t\t";
                    for (unsigned int r = 0; r < DENDROSOLVER_GW_NUM_RADAII;
                         r++)
                        fileGW << "r" << r << "\t";
